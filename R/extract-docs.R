# --- Vision extraction: PDF to markdown ---
...
extract_via_vision <- function(pdf_path, path_out, api_key = Sys.getenv("GOOGLE_AI_KEY")) {
    ...
    prompt <- paste(
        ...
        "  Do not use markdown headers except for section titles marked with roman numerals.",
        "4. Convert any data table (including image-based tables) to markdown table format.",
        "   Ensure that the header row is first.",
